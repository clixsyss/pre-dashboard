...
const NewsCategoriesManagement = ({ projectId }) => {
  ...
  // Available icon options for news
  const iconOptions = [
    { value: '📰', label: 'News', icon: '📰' },
    { value: '📢', label: 'Announcement', icon: '📢' },
    { value: '🎉', label: 'Event', icon: '🎉' },
    { value: '📅', label: 'Calendar', icon: '📅' },
    { value: '⚠️', label: 'Alert', icon: '⚠️' },
    { value: '🏆', label: 'Achievement', icon: '🏆' },
    { value: '💼', label: 'Business', icon: '💼' },
    { value: '🎓', label: 'Education', icon: '🎓' },
    { value: '🏃', label: 'Sports', icon: '🏃' },
    { value: '🎨', label: 'Culture', icon: '🎨' },
    { value: '🔧', label: 'Maintenance', icon: '🔧' },
    { value: 'ℹ️', label: 'Info', icon: 'ℹ️' }
  ];
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      alert('Failed to load news categories');
    } finally {
      setLoading(false);
    }
  }, [projectId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCategory) {
        ...
        const categoryRef = doc(db, `projects/${projectId}/newsCategories`, editingCategory.id);
  ...
  const handleDelete = async (categoryId) => {
    ...
    try {
      await deleteDoc(doc(db, `projects/${projectId}/newsCategories`, categoryId));
  ...
  const toggleStatus = async (category) => {
    try {
      const categoryRef = doc(db, `projects/${projectId}/newsCategories`, category.id);
  ...
  if (loading && categories.length === 0) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-3 text-gray-600">Loading news categories...</span>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage news categories that can be used when creating news posts
          </p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {categories.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">Create your first news category to get started</p>
            <button
              onClick={() => setShowAddModal(true)}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., فعاليات المجتمع"
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  dir="rtl"
                />
              </div>
              ...
              <div className="flex items-center">
                ...
                <label htmlFor="isActive" className="ml-2 block text-sm text-gray-900">
                  Active (visible when creating news posts)
                </label>
              </div>
